...
ICACHE_FLASH_ATTR size_t strnlen_P(PGM_P s, size_t size) {
    const char* cp;
    for (cp = s; size != 0 && pgm_read_byte(cp) != '\0'; cp++, size--);
    return (size_t) (cp - s);
}

ICACHE_FLASH_ATTR char* strstr_P(const char* haystack, PGM_P needle)
{
...
ICACHE_FLASH_ATTR int memcmp_P(const void* buf1, PGM_VOID_P buf2P, size_t size) {
    int result = 0;
...
ICACHE_FLASH_ATTR void* memccpy_P(void* dest, PGM_VOID_P src, int c, size_t count) {
...
ICACHE_FLASH_ATTR void* memmem_P(const void* buf, size_t bufSize, PGM_VOID_P findP, size_t findPSize) {
...
ICACHE_FLASH_ATTR char* strncpy_P(char* dest, PGM_P src, size_t size) {
    const char* read = src;
    char* write = dest;
    char ch = '.';
    while (size > 0 && ch != '\0')
...
ICACHE_FLASH_ATTR char* strncat_P(char* dest, PGM_P src, size_t size) {
    char* write = dest;
...
ICACHE_FLASH_ATTR int strncmp_P(const char* str1, PGM_P str2P, size_t size) {
    int result = 0;
...
ICACHE_FLASH_ATTR int strncasecmp_P(const char* str1, PGM_P str2P, size_t size) {
    int result = 0;
...
ICACHE_FLASH_ATTR int printf_P(PGM_P formatP, ...) {
    int ret;
    va_list arglist;
...
ICACHE_FLASH_ATTR int sprintf_P(char* str, PGM_P formatP, ...) {
    int ret;
    va_list arglist;
    va_start(arglist, formatP);
...
ICACHE_FLASH_ATTR int snprintf_P(char* str, size_t strSize, PGM_P formatP, ...) {
    int ret;
    va_list arglist;
    va_start(arglist, formatP);
...
ICACHE_FLASH_ATTR int vsnprintf_P(char* str, size_t strSize, PGM_P formatP, va_list ap) {
    int ret;
