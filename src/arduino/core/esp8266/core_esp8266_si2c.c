...
ICACHE_FLASH_ATTR  void twi_setClock(unsigned int freq){
#if F_CPU == FCPU80
...
ICACHE_FLASH_ATTR void twi_setClockStretchLimit(uint32_t limit){
  twi_clockStretchLimit = limit * TWI_CLOCK_STRETCH_MULTIPLIER;
}

ICACHE_FLASH_ATTR void twi_init(unsigned char sda, unsigned char scl){
  twi_sda = sda;
  twi_scl = scl;
...
ICACHE_FLASH_ATTR void twi_stop(void){
